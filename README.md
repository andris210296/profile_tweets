...
There are two bugs happening in this project. The first one happens in the front-end part when you open the Update Profile Form, for some reason unknown
the layout, colors e design get lost, but the functionality still works. The second bug happens when the page tries to retrieve the storage image from the computer, the
save (upload) part works properly, but the image doesn't show in the page.
...
contains those information, but in the future the given datas can expire. Also change the location to store the uploaded images.

**Starting the profile_tweets**

This code was developed using IntelliJ, which can start it easily after doing the configurations above.
...
**a)** In order to create a portfolio create a **POST** request at the endpoint **{server}/create** with a body as described below:
...
The only unpossible changes are the Tweets itself, because it's automatically pushed from the Twitter API, however you can change the **twitterUserName** in which the 
...
In order to use th front-end, only start the application, open a browser and tap **{server}/**
...
informations and then click **SAVE**. The created Portfolio will be presented in the index page right after it's creation.
...
and click **SAVE** or press **Choose a file** and select an image and then Upload in order to add a image in your portfolio. The changes will be presented in the index page

**c)** To delete a portfolio, click the **DELETE** button, after that the portfolio will desapear. 
